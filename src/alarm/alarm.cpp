...
void AlarmMgr::set_alarm(std::string alarmid, int dow, tm* alarm_time, tm* start_date, tm* stop_date) {
    ...
    ins.bind(1+(int)col_names::dow,         dow);
...
void AlarmMgr::deactivate_alarm(std::string alarmid, bool snooze)
{
    ...
    if(snooze)
    {
        ...
        int dow = alarm_time.tm_wday;

        struct tm end_date = *localtime(&rawtime);
        end_date.tm_mday = end_date.tm_mday + 1;

        set_alarm(alarmid+"-snoozed", dow, &alarm_time, &start_date, &end_date);
    }
}
